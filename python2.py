...
def analyze_for_all():
   ...
   #  print(resultSources)
    ...
    for i in range(0, len(responsePerAnswer)):
        for k in range(0, len(responsePerAnswer[i])):
            for p in range(0, len(responsePerAnswer[i][k])):
                print('i : ', i, '|', 'k ', k, 'p', p, '|', responsePerAnswer[i][k][p][i], '|', responsePerAnswer[i][k][
                    p])  # i :  4(문제 번호수) | k(답변 번호수)  2 p(갯수에 따른 분류) 0 | 4_기권 | ['0_남성', '1_짬뽕', '2_싫음', '3_찬성', '4_기권']
    ...
    '''
    new_data = [[elem.replace("'", "") if isinstance(elem, str) else elem for elem in row] for row in result_list]
    return new_data
